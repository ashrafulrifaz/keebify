...
import ProductCategorySelector from '../AddProduct/ProductCategorySelector';
import ProductStatusSelector from '../AddProduct/ProductStatusSelector';
...
const EditProduct = ({slug}) => {
    ...
    const editingProduct = products?.find(p => p._id === slug) || {}

    const [productCategory, setProductCategory] = useState(editingProduct?.category || null)
    const [productStatus, setProductStatus] = useState(editingProduct?.status || null)
    const [colors, setColors] = useState(editingProduct?.colors || []);

    useEffect(() => {
        if (editingProduct) {
            setColors(editingProduct.colors || []);
            setProductCategory(editingProduct.category || null);
            setProductStatus(editingProduct.status || null);
            setExistingPhotos(editingProduct.photos || []);
        }
    }, [editingProduct?._id]);
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            const product = {
                ...
                price: data.price ? data.price : editingProduct?.price,
        ...
        <form onSubmit={handleSubmit(onSubmit)} className='mt-5 bg-border/50 rounded-xl px-4 py-5'>
            <div className='grid grid-cols-3 gap-5'>
                <div>
                    ...
                    <input type="text" id='name' placeholder='Enter product name' className='mt-3 border border-[#c4c4c4]/50 rounded-lg py-2 px-3 text-[15px] focus:outline-0 w-full' defaultValue={editingProduct?.name} {...register("name")} />
                </div>
                <div>
                    <label htmlFor="price" className='text-[#414141] block font-medium'>Product Price</label>
                    <input type="number" id='price' placeholder='Enter product price' className='mt-3 border border-[#c4c4c4]/50 rounded-lg py-2 px-3 text-[15px] focus:outline-0 w-full' defaultValue={editingProduct?.price} {...register("price")} />
                </div>
                <div>
            ...
            <div className='grid grid-cols-4 gap-5 mt-5'>
                ...
                <div>
                    ...
                    <ProductCategorySelector productCategory={productCategory} setProductCategory={setProductCategory} />
                </div>
                <div>
                    <label htmlFor="category" className='text-[#414141] block font-medium'>Product Status</label>
                    <ProductStatusSelector productStatus={productStatus} setProductStatus={setProductStatus} />
                </div>
            </div>
            <div className='grid grid-cols-2 gap-5 mt-5'>
                <div>
                    <label htmlFor="description" className='text-[#414141] block font-medium'>Product Description</label>
                    <textarea type="number" id='description' rows={10} placeholder='Enter product total reviews' className='mt-3 border border-[#c4c4c4]/50 rounded-lg py-2 px-3 text-[15px] focus:outline-0 w-full'defaultValue={editingProduct?.description} {...register("description")} />
                </div>
                <div>
                            ...
                            {existingPhotos.map((url, index) => (
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <p className="text-[14px] text-[#1a1a1a] truncate">
                                            {url.split('/').pop()}
                                        </p>
                                        <p className="text-[12px] text-[#696969]">Already uploaded</p>
                                    </div>
                                    <button
                                        type="button"
            ...
            <div>
                <button disabled={(existingPhotos.length === 0 && files.length === 0)} type="submit" className={`capitalize py-3 px-20 cursor-pointer rounded-xl mt-6 font-medium ${(existingPhotos.length === 0 && files.length === 0) ? 'bg-primary/80' : 'bg-primary text-white'}`}>Update Product</button>
            </div>
        </form>
    );
};
